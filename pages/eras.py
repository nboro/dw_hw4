...
era_df = bill_join_df.groupby(["title", "era", "hp"]).count().reset_index()[["title", "era", "hp", "artist"]].rename(
    columns={"artist": "count"}
)
print("TEST")
# print(era_df.groupby("era").count())
# era_df["era_rank"] = era_df.groupby("era").rank(method="first")["hp"]
# best_era_df = bill_join_df[bill_join_df["title"].isin(era_df[era_df["era_rank"] < 200]["title"].unique())].copy()[["title", "era"] + song_features].dropna().reset_index()
